      ...
      <section className="pt-32 pb-16 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            ...
            <h1 className="font-display font-black text-6xl md:text-7xl mb-6">Portfolio</h1>
            <div className="flex flex-wrap gap-2 mt-8">
              {FILTERS.map(f => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-5 py-2 rounded-full text-sm font-semibold capitalize transition-all duration-200 ${filter === f ? 'bg-foreground text-primary-foreground' : 'border border-border hover:border-foreground'}`}
                >
                  {f === 'all' ? 'All Work' : f}
                </button>
              ))}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((item, i) => (
                <Link
                  ...
                  className={`group relative rounded-2xl overflow-hidden bg-muted ${i === 0 ? 'md:col-span-2 aspect-[16/7]' : 'aspect-[4/3]'}`}
                >
                  {item.images?.[0] ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end p-6">
                    <div className="flex items-end justify-between w-full">
                      <div>
                        ...
                        <div className="font-display font-bold text-white text-2xl">{item.title}</div>
